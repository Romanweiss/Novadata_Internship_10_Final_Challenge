...
"""
DAG ежедневного пересчёта ETL-витрины и проверки выгрузки в S3.

Почему в этом файле много Bash-вызовов:
1. Проект работает в Docker-only режиме, а Airflow здесь выступает как оркестратор,
   а не как вычислительный движок.
2. Тяжёлая логика ETL уже реализована и отлажена внутри контейнера `app`
   (Python/Spark-окружение, .env, зависимости, JDBC-драйверы).
3. Чтобы не дублировать среду и не переносить ETL-код в сам Airflow-контейнер,
   DAG запускает команды через `BashOperator` и `docker exec` именно в `app`.
4. Через Bash также удобно выполнять инфраструктурные проверки:
   - ожидание готовности ClickHouse (`/ping`);
   - запуск ETL-скрипта в рабочем каталоге проекта;
   - пост-проверку, что файл реально появился в S3 и имеет корректную структуру.

Итоговая цепочка выполнения:
Airflow (scheduler) -> BashOperator -> docker exec -> app container -> ETL/Python проверки.

Это стандартный паттерн для учебных и production-like DE-пайплайнов в контейнерной среде:
оркестрация в Airflow, вычисления и I/O — в специализированном runtime контейнере.
"""
