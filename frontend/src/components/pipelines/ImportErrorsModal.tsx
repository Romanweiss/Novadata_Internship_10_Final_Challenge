...
import type { ApiImportRowError } from '../../api/client';
        ...
        >
          ...
          >
            <div className="mb-4 flex items-start justify-between gap-4">
              <div>
                <h3 className="text-2xl font-extrabold leading-tight">{t('pipelines.import.errorsModal.title')}</h3>
                <p className="mt-1 text-[0.95rem] text-[var(--text-muted)]">
                  {t('pipelines.import.errorsModal.subtitle')}
                </p>
                {total > 0 ? (
            ...
            {!loading && !errorMessage && items.length === 0 ? (
              <p className="text-sm text-[var(--text-muted)]">{t('pipelines.import.errorsModal.empty')}</p>
            ) : null}
              ...
              <div className="max-h-[60vh] overflow-auto rounded-2xl border border-[var(--border)]">
                <table className="min-w-full text-sm">
                  <thead className="sticky top-0 bg-[var(--surface-muted)] text-[var(--text-muted)]">
                    <tr>
                      <th className="px-3 py-2 text-left font-semibold">{t('pipelines.import.errorsModal.row')}</th>
                      <th className="px-3 py-2 text-left font-semibold">{t('pipelines.import.errorsModal.field')}</th>
                      <th className="px-3 py-2 text-left font-semibold">{t('pipelines.import.errorsModal.code')}</th>
                      <th className="px-3 py-2 text-left font-semibold">{t('pipelines.import.errorsModal.message')}</th>
                      <th className="px-3 py-2 text-left font-semibold">{t('pipelines.import.errorsModal.raw')}</th>
                    </tr>
                  </thead>
                  <tbody>
                    {items.map((item) => (
                      <tr key={item.id} className="border-t border-[var(--border)] align-top">
                        ...
                        <td className="whitespace-nowrap px-3 py-2">{item.field_name || '-'}</td>
                        <td className="whitespace-nowrap px-3 py-2 font-mono text-xs">{item.error_code}</td>
            ...
            <div className="mt-5 flex justify-end">
              ...
              >
                {t('pipelines.import.errorsModal.close')}
              </button>
            </div>
          </motion.div>
        </motion.div>
